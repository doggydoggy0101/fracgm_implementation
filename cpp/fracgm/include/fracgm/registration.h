...
class registration {
    ...
    registration(const int &max_iteration, const double &tol, const double &c, const double &noise_bound);

    std::vector<double> solve_beta_mu(std::vector<Fractional> *terms);

    Eigen::VectorXd solve_x(std::vector<double> *alpha, std::vector<Fractional> *terms);
