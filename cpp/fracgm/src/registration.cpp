...
registration::registration(const int &max_iteration, const double &tol, const double &c, const double &noise_bound) {
...
std::vector<double> registration::solve_beta_mu(std::vector<Fractional> *terms) {
...
Eigen::VectorXd registration::solve_x(std::vector<double> *alpha, std::vector<Fractional>  *terms) {

    Eigen::MatrixXd mat_a = Eigen::MatrixXd::Zero(13, 13);
    int n = terms->size();

    for (int i = 0; i < n; i++) {
        mat_a += alpha->at(n + i) * terms->at(i).f_mat() - alpha->at(n + i) * alpha->at(i) * terms->at(i).h_mat();
    }
...
float registration::compute_psi_norm(std::vector<double> *alpha, std::vector<Fractional> *terms) {
    float a;
    float b;
    int n = terms->size();
...
void registration::update_terms_cache(std::vector<Fractional> *terms, Eigen::VectorXd *vec){
    for (auto& term : *terms) {
        term.update_cache(*vec);
    }
}

Eigen::Matrix4d registration::solve(const PointCloud &pcd1, const PointCloud &pcd2) {
    ...
    std::vector<Fractional> terms = compute_terms(pcd1, pcd2, noise_bound_*noise_bound_, c_*c_);
